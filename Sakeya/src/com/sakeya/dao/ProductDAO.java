...
//	 관리자 모드에서 사용하는 메소드 
	...
	public String pageNumber(int tpage, String name) {
		...
		if (start_page > view_rows) {
			str += "<a href='SakeyaServlet?command=admin_product_list&tpage=1&key=" + name
					+ "'>&lt;&lt;</a>&nbsp;&nbsp;";
			str += "<a href='SakeyaServlet?command=admin_product_list&tpage=" + (start_page - 1);
		...
		for (int i = start_page; i <= end_page; i++) {
			if (i == tpage) {
				...
				str += "<a href='SakeyaServlet?command=admin_product_list&tpage=" + i + "&key=" + name + "'>[" + i
						+ "]</a>&nbsp;&nbsp;";
			}
		}
		if (page_count > end_page) {
			str += "<a href='SakeyaServlet?command=admin_product_list&tpage=" + (end_page + 1) + "&key=" + name
					+ "'> &gt; </a>&nbsp;&nbsp;";
			str += "<a href='SakeyaServlet?command=admin_product_list&tpage=" + page_count + "&key=" + name
					+ "'> &gt; &gt; </a>&nbsp;&nbsp;";
		}
		return str;
	}
	...
	public ArrayList<ProductVO> listProduct(int tpage, String product_name) {
		ArrayList<ProductVO> productList = new ArrayList<ProductVO>();
		String str = "select pseq, indate, name, price1, price2, useyn, bestyn FROM product "
				+ "WHERE upper (name) LIKE upper ('%'||?||'%') order by pseq desc"; 
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int absolutepage = 1;
